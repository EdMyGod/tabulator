...
### Пресеты колонок Cubisio

Для типовых финансовых таблиц доступны готовые конструкторы колонок:

```js
Tabulator.cubisioMoneyColumn(year + 1, "ЗБП_1год")

Tabulator.cubisioDynamicPercentColumn(
    year + 1,
    "del1",
    "ЗБП_1год",
    "ЗБ_1год"
...
Tabulator.cubisioDynamicAbsoluteColumn(
    year + 1,
    "abs1",
    "ЗБП_1год",
    "ЗБ_0год"
...
Последним аргументом можно передать объект стандартных настроек Tabulator, который переопределяет настройки пресета:
...
Процентная динамика рассчитывается как `(current / previous - 1) * 100`, абсолютная — как `current - previous`. Итоги автоматически считаются по выбранным строкам, если выбор есть, иначе по всем строкам текущего набора данных. При нулевой базе процентная динамика возвращает пустое значение.
